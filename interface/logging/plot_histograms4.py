...
# Define function to calculate Inter Quartile Mean
def inter_quartile_mean(series):
    q1 = series.quantile(0.25)
    q3 = series.quantile(0.75)
    return series[(series >= q1) & (series <= q3)].mean()

# Aggregate feedbacks per servo and group_id using Inter Quartile Mean
aggregated_feedback = feedbacks_long.groupby(['servo_num', 'group_id']).agg({
    'command_angle': 'first',
    'feedback': inter_quartile_mean
}).reset_index()
